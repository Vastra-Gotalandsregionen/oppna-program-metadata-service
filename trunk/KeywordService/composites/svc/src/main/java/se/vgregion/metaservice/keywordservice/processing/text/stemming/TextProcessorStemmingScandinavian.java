...
public abstract class TextProcessorStemmingScandinavian extends TextProcessor {
    ...
    public ProcessorStatus process(AnalysisDocument document) {
        ...
        StringBuffer processedString = null;
    ...
    protected String replaceSuffix(String word, String r1, Map<String, String> suffixesReplace) {
        for (Map.Entry<String, String> suffixReplace : suffixesReplace.entrySet()) {
            if (r1.endsWith(suffixReplace.getKey())) {

                word = word.substring(0,
                        word.length() - suffixReplace.getKey().length()).concat(suffixReplace.getValue());
                log.debug(MessageFormat.format(
